	...
	public function validateParameter($fieldName, $value, $dataType, $required = true) {
 	...
 	public function processApi() {
			...
			try {
				...
				$stmt = $this->dbConn->prepare("SELECT * FROM tbl_user_info WHERE user_id = :userId");
		...
		public function throwError($code, $message) {
            ...
            $arrytest = array_merge(array('error'=>array('code'=>400, 'message' =>$message)),array('duplicates'=>$duplicates),array('success'=>$success));
		...
		public function returnResponse($code, $data) {
			header("content-type: application/json");
			$arrytest = array_merge(array('response'=>array('code'=>$code, 'result' =>$data)));
            $response = json_encode($arrytest);
            echo $response; exit;
			}
